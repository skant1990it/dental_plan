...
class IndexController extends AbstractActionController
{
          ...
          $data = (array) $data1;
           ...
           $authService = $this->getServiceLocator()->get('AuthService');
            $userdetail  =  $this->getServiceLocator()->get("UserTable")->authenticateDoctor($data['userName'] ,$encyptPass);
             ...
             $loginstepcomplete = $userdetail['0']['login_step_complete'] ;
                if (isset($resultid)&&($loginstepcomplete==1)) {
                   ...
                   if($userdetail[0]['all_steps_complete']==0){
					    if($userdetail[0]['doc_speciality']==3){
							echo '4';exit;   // here 4 for doctor specility redirect.
						}else
							echo '2';exit;   // here 2 for goto the banking page.
 ...
 public function registerAction()
  {     
        ...
        if(isset($userdetail[0]['doc_username'])&&($userdetail[0]['login_step_complete']==1))
                 { 
                    $exists=1 ;
  ...
  {   
      ...
      $doctordetail = $this->getServiceLocator()->get('Dashboardfront\Model\DashboardfrontTable')->doctorDetailsFromId($docid);
      if($doctordetail[0]['doc_speciality']==3){
      $updateloginstepcomplete = $this->getServiceLocator()->get('Dashboardfront\Model\DashboardfrontTable')->updateDoctorLoginStepComplete($docid);
      }
    }
      echo 'true';die; 
   }
  ...
  public function createaccountAction()
    ...
    public function submitBillingAction()
    {
     ...
     $data = (array) $data1;
     $docid=$_COOKIE['useridafterregistration']; 
     $result=$this->getServiceLocator()->get('Dashboardfront\Model\DashboardfrontTable')->updateDocBilling($docid,$data); return new ViewModel();
    }
     public function submitlocationAction()
    {
      ...
      $docid=$_COOKIE['useridafterregistration']; 
      $userdetail = $this->getServiceLocator()->get('Dashboardfront\Model\DashboardfrontTable')->updateDoctorDetailsLocation($docid,$legalbusinessname,$practice,$kind_of_practice,$doctraveltype);
      ...
      $resultinsertlocdetails= $this->getServiceLocator()->get('Dashboardfront\Model\DashboardfrontTable')->insertDocLocation($docid,$data,$doctraveltype);
